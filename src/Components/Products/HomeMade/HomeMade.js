...
import HomemadeImages from "../../Images/HomeMade";
...
const HomeMade = () => {
  ...
  return (
    <div id="HomemadeSection" className="mt-4">
      <Container className="Products-Container">
        <Row className="Products-Row-1">
          <h2>Homemade Drinks</h2>
          {!isLoading && !isLoading2 ? (
            <>
              {data[5]?.products?.map((dataofHomeMade, id) => (
                <>
                  <Col
                    className="Products-Col-1"
                    key={id}
                    sm={5}
                    onClick={() => ModalValue(dataofHomeMade)}
                  >
                    <Row>
                      <Col xs={7}>
                        <div className="p-3 Products-Col-1-div-1">
                          {" "}
                          <h5>{dataofHomeMade.name}</h5>
                          <p className="Products-descr-para">{dataofHomeMade.description}</p>
                          <div className="Product-Price">
                            <span>
                              <BsPlusCircle style={{ color: "06B906" }} /> ${" "}
                              {dataofHomeMade.price}
                            </span>
                      ...
                      <Col xs={5}>
                        <img
                          ...
                          src={`${data2?.base_urls?.product_image_url}/${dataofHomeMade.image}`}
                          // src={dataofHomeMade.image}
                          alt="Products"
                        />
                      </Col>
                    </Row>
